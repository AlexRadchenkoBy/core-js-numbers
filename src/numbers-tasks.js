...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const modules1 = Math.sqrt(x1 * x1 + y1 * y1);
  const modules2 = Math.sqrt(x2 * x2 + y2 * y2);
  const cosAngle = scalarProductOfVectors / (modules1 * modules2);
  const invalidValue = Math.max(-1, Math.min(1, cosAngle));
  return Math.acos(invalidValue);
}
...
function roundToPowerOfTen(num, pow) {
  const calculateMultiplier = 10 ** pow;
  return Math.round(num / calculateMultiplier) * calculateMultiplier;
}
